...
angular.
module('hospital', ['service', 'service.user', 'datatables', 'datatables.bootstrap', 'datatables.buttons', 'ui.bootstrap', 'ui.bootstrap.tpls'])
    .component('hospital', {
        templateUrl: 'hospital/hospital.template.html',

        //三个DT derective的引入次序必须在$q之前而且有先后顺序,否则会报错
        //数组前几个元素为provider                                                           //再次声明,上下两行的对应关系一定要严格执行,先后次序也不能错
        controller: ['DTOptionsBuilder', 'DTColumnBuilder', 'DTColumnDefBuilder', '$q', '$scope', '$compile', '$http', '$sce', 'urlconfig',
            function hospitalController(DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $q, $scope, $compile, $http, $sce, urlconfig) {
                //定义变量
                var self = this;
                var dtInstance = {};
                var message = '';
                ...
                //定列名
                self.dtColumns = [
                    DTColumnBuilder.newColumn('hospitalNumber').withTitle('hospitalNumber').withOption('sWidth', '10%'),
                    DTColumnBuilder.newColumn('hospitalName').withTitle('hospitalName').withOption('sWidth', '20%'),
                    DTColumnBuilder.newColumn('hospitalPhone').withTitle('hospitalPhone').withOption('sWidth', '15%'),
                    DTColumnBuilder.newColumn('hospitalAddress').withTitle('hospitalAddress').withOption('sWidth', '25%'),
                    DTColumnBuilder.newColumn(null).withTitle('Actions').notSortable().renderWith(actionsHtml)
                ];

                var showdata = '';
                self.edit = function(hospital) {
                    ...
                    self.message = hospital;
                    self.showdata = hospital;
                ...
                function actionsHtml(data, type, full, meta) {
                    self.hospital[data.hospitalNumber] = data; //把返回的所有数据装进一个数组,数组索引为数据的userID字段
                    ...
                    return '<button class="btn btn-default" data-toggle="modal" data-target="#myModal" ng-click="$ctrl.edit($ctrl.hospital[' + data.hospitalNumber + '])"><span class="glyphicon glyphicon-cog" aria-hidden="true"></span> edit </button>&nbsp;' +
                        //return '<button class="btn btn-warning" data-toggle="modal" data-target="#myModal" > use </button>&nbsp;' +
                        '<button class="btn btn-danger" ng-click="$ctrl.deleteRow($ctrl.license[' + hospital.hospitalNumber + '])" )"=""><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> delete</button>';
